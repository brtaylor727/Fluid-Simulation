...
if __name__ == '__main__':
    ...
    files = sorted(glob.glob(sys.argv[1]+"*.dat"))
    ...
    x = np.linspace(0,dx*L,L)
    plt.scatter(np.linspace(0,dx*L,L),myinfo[0]['a'][:,-1],label="Simulation-start")
    plt.scatter(np.linspace(0,dx*L,L),myinfo[-1]['a'][:,-1],label="Simulation-emd")
    plt.plot(x,nu*phi*x*(x-L*dx)/(2*rho),label='Theory')
    plt.legend()
    plt.xlim(left = 0)
    plt.xlim(right = dx*L)
    plt.show()
